...
`web/lib/*` re-exports from this package for Next.js routes.
...
node packages/ai-worker/dist/server.mjs
```

After changing worker logic in `web/` (before sync), run:

```bash
node packages/ai-worker/scripts/sync-from-web.mjs
```

## Docker

```bash
docker build -f packages/ai-worker/Dockerfile -t voice-inbox-ai-worker .
