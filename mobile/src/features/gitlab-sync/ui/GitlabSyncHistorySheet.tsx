...
import type { GitlabCommitSummary } from '../lib/gitlabApi';
...
type Props = {
  ...
  onRestore: (commit: GitlabCommitSummary) => void;
};
      ...
      >
        ...
        >
          <Pressable
            ...
            accessibilityLabel={t('settings.gitlabSync.restoreVersionA11y', { title })}
          ...
          {commit.htmlUrl ? (
            <Pressable
              ...
              accessibilityRole="button"
              accessibilityLabel={t('settings.gitlabSync.openOnGitlab')}
              hitSlop={4}
              style={({ pressed }) => ({
                alignItems: 'center',
                backgroundColor: color.background.tertiary,
...
export function GitlabSyncHistorySheet({
  visible,
  color,
  commits,
  loading,
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: GitlabCommitSummary; index: number }) => (
      <HistoryListRow
        color={color}
        commit={item}
        isLast={index === commits.length - 1}
        title={item.message || t('settings.gitlabSync.untitledCommit')}
  ...
  const keyExtractor = useCallback((item: GitlabCommitSummary) => item.sha, []);
  ...
  const listBody = loading ? (
    ...
    >
      {t('settings.gitlabSync.historyEmpty')}
    </Text>
  ) : (
    <View
      className="w-full"
  ...
  return (
    <AppBottomSheetModal visible={visible} onClose={onClose}>
      <AppBottomSheetContent bottomPadding={12} style={{ paddingTop: 4 }}>
        <SheetHeader
          title={t('settings.gitlabSync.historyTitle')}
          subtitle={t('settings.gitlabSync.historySubtitle')}
          color={color}
          marginBottom={10}
        />
