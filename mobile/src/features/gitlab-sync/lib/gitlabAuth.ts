import { getGitlabOAuthClientId as getGitlabOAuthClientIdFromConfig } from '@/shared/config/runtimeConfig';
import { nitroFetch } from '@/shared/lib/fetch';
import { isRecord, isString } from '@/shared/lib/type-guards';

import { GITLAB_ACCESS_TOKEN_URL, GITLAB_DEVICE_CODE_URL, GITLAB_OAUTH_SCOPE } from './constants';
...
export type { GitlabDeviceFlowChallenge } from './gitlabVerificationUri';
export { buildGitlabVerificationUriComplete } from './gitlabVerificationUri';
...
export function cancelGitlabDeviceFlow(): void {
...
export function getGitlabOAuthClientId(): string {
  return getGitlabOAuthClientIdFromConfig();
}

export function isGitlabOAuthConfigured(): boolean {
  return getGitlabOAuthClientId().length > 0;
}
...
async function pollAccessToken(
  ...
  while (Date.now() < deadline) {
    ...
    const response = await nitroFetch(GITLAB_ACCESS_TOKEN_URL, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
...
export async function startGitlabDeviceFlow(
  onChallenge?: (challenge: GitlabDeviceFlowChallenge) => void,
): Promise<GitlabDeviceFlowChallenge> {
  const clientId = getGitlabOAuthClientId();
  if (!clientId) {
  ...
  try {
    ...
    const challenge = toGitlabDeviceFlowChallenge(device);
    ...
    await setGitlabSyncAccessToken(accessToken);
...
export async function revokeGitlabConnection(): Promise<void> {
