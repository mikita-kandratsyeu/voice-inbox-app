...
describe('shouldRunTranscriptionDiarization', () => {
  it('runs for meeting records when Pro is active and WhisperKit is enabled', () => {
    expect(shouldRunTranscriptionDiarization({ classification: 'meeting' }, true)).toBe(true);
  });

  it('skips non-meeting records', () => {
    expect(shouldRunTranscriptionDiarization({ classification: 'work' }, true)).toBe(false);
    expect(shouldRunTranscriptionDiarization({}, true)).toBe(false);
  });

  it('never runs without Pro', () => {
    expect(shouldRunTranscriptionDiarization({ classification: 'meeting' }, false)).toBe(false);
  });
