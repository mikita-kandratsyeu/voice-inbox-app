...
import { KeyRound, MoreVertical, Trash2, X } from 'lucide-react';
...
export function AdminUserRowMenu({ isCurrent, isEditing, isDeleting, onEdit, onDelete }: Props) {
  ...
  return (
    <div className="relative shrink-0" ref={rootRef}>
      <button
        ...
        className={`${adminBtnGhostClass} px-2`}
      >
      ...
      {open ? (
        <div
          id={menuId}
          role="menu"
          className="absolute right-0 top-full z-20 mt-1 min-w-[11rem] rounded-lg border border-zinc-200 bg-white py-1 shadow-lg shadow-zinc-950/10 dark:border-zinc-600 dark:bg-zinc-900 dark:shadow-black/40"
        >
          <button
            type="button"
            role="menuitem"
          ...
          >
            ...
            {isEditing ? 'Close editor' : 'Edit access'}
          </button>
          {!isCurrent ? (
            <button
              type="button"
