      ...
      <div className="mx-auto max-w-4xl">
        <AnimateOnScroll>
          <div className="mb-10 text-center sm:mb-12">
            ...
            <p className="text-base text-black/60 sm:text-lg dark:text-white/60">{t('subtitle')}</p>
          </div>
        </AnimateOnScroll>
        ...
        <div className="space-y-3 sm:space-y-4">
          {FAQ_ITEMS.map((key, index) => (
            <AnimateOnScroll key={key} delay={index * 70}>
              <details
                className="group rounded-2xl border border-black/10 bg-white/85 shadow-[0_8px_26px_rgba(15,23,42,0.05)] dark:border-white/12 dark:bg-white/5 dark:shadow-[0_10px_30px_rgba(0,0,0,0.3)]"
                open={index === 0}
              >
