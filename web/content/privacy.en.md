...
**Public note links (optional in Pro):** If you choose **Publish note** in the app share sheet, the selected note snapshot (title + chosen markdown format) is uploaded to our servers to create a public link. The link is accessible to anyone who has it. You can set an expiry period or unpublish it at any time from the app. When you unpublish or the link expires, the snapshot is **deleted from our servers**. We do not keep a copy afterward.
...
**GitLab note sync (optional, Pro):** If you connect GitLab under **Backup & restore** in Settings, the app uses **GitLab’s OAuth Device Flow** on **gitlab.com** to obtain an access token. The token and your chosen project settings are stored **only on your device** (in the system keychain). When you sync, the app sends **markdown and metadata** (for example folders and graph layout) — **not audio** — directly to **your GitLab project** via the GitLab API. We do not receive, store, or process synced note content on our servers. GitLab processes this data under [GitLab’s privacy policy](https://about.gitlab.com/privacy/). You can disconnect GitLab in Settings at any time, which removes the token from your device. GitHub and GitLab sync are independent optional features; you may use either, both, or neither.
...
**GitLab (optional):** Connecting GitLab is separate from a Voice Inbox account (there is none). It authorizes the app to access GitLab projects you select on **gitlab.com** under the OAuth scopes shown in the app (`api`, `read_user`, `write_repository`) solely to sync or restore your notes as described under “GitLab note sync” in section 1.
...
**On your device:** recordings, transcripts, and AI-generated content are stored until you delete them or uninstall the app. If you connect GitHub or GitLab, your OAuth access token and sync preferences for that provider are stored in the system keychain until you disconnect that provider in Settings or uninstall the app.
...
**In your GitLab account:** if you use GitLab note sync, copies of exported note data remain in the project you chose on gitlab.com, according to GitLab’s retention and your own actions (commits, branches, deletion).

**On our servers:** Temporary AI job results in our key-value cache are retained until they expire (up to one hour by default; the app may request as little as five minutes, as described in section 2). Per-device **weekly usage counters** for AI credits are stored in a short-lived key-value store for up to 8 days, then reset. **AI credit ledger entries** and **records of one-time limit-restoration purchases** are stored in our database for as long as needed to show your usage history, provide paid features, and prevent abuse (including reuse of the same store transaction). Push tokens are stored for up to 30 days and refreshed on every app launch with notifications enabled. **Support requests** (including optional contact details and diagnostics you send through the support form) are stored in our database as described in section 2. **Subscription and other in-app purchase records** needed to verify entitlements (for example, device identifier, entitlement expiry, and store transaction identifiers) are stored as long as needed for those purposes. **Published note snapshots** are kept only while the link is active; when you unpublish or the link expires, they are **deleted from our servers**. **Android waitlist** submissions are retained as long as needed to run the waitlist and send related emails, unless you ask us to delete your entry or applicable law requires otherwise.
...
- **Export** — data can be exported from the app at any time (including ZIP backup and, if you are eligible and choose to connect GitHub and/or GitLab, optional sync to your repository or project).
...
- **Disconnect GitLab** — remove the GitLab connection in Settings to delete the stored token from your device (copies already pushed to GitLab remain under your GitLab account until you delete them there).
