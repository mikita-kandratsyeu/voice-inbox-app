...
**Voice Inbox AI Pro (license keys):** If you activate **Voice Inbox AI Pro** using a license key, the app sends the key to our API over HTTPS. We **do not** store the key in readable form on our servers; we store a **cryptographic hash** of the key for validation, the **activation time**, and associate the entitlement with your **app device identifier** (the same identifier used for API access and rate limiting — see above). We store the **expiry time** of your Pro access so we can enforce limits and support. Pro status may also be **cached locally** on your device (for example, to show your expiry date offline). **Each license key can only be activated on one device**; the same key cannot be used to activate Pro on a second device.
