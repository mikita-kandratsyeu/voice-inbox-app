...
Privacy and Terms describe **cloud AI** (OpenRouter; providers such as Google / Gemini, DeepSeek, MiniMax, Xiaomi / MiMo, StepFun, and NVIDIA / Nemotron), **optional on-device / private AI** (local inference; model files may be downloaded from hosts such as Hugging Face), optional push tokens, **in-app support** (optional email + message + technical diagnostics stored in our database), **website Android waitlist** (optional name, email, device info via a third-party form; beta invites and occasional product emails), **mobile advertising (Yandex Mobile Ads)** for users without a paid ad-free entitlement, and **paid features via App Store / Google Play in-app purchases** (with optional subscription-status verification). Wording is time-stamped via “Last updated” and may change. Keep in sync with mobile: `mobile/src/shared/lib/ai-core/orchestrator.ts` (execution mode), `mobile/src/screens/settings/ui/PrivateAiModeScreen.tsx`, `mobile/src/entities/settings/model/constants.ts` (cloud + local model catalogs), `mobile/src/features/app-storefront/model/useAdsAllowed.ts`, `mobile/src/features/yandex-interstitial` (interstitial placements + caps), and `mobile/src/features/tech-support`.
