...
**Voice Inbox AI Pro (лицензионные ключи):** если вы активируете **Voice Inbox AI Pro** с помощью лицензионного ключа, приложение отправляет ключ на наш API по HTTPS. Мы **не** храним ключ на серверах в открытом виде; хранится **криптографический хеш** ключа для проверки, **время активации** и привязка права к **идентификатору устройства приложения** (тому же, что используется для доступа к API и лимитов — см. выше). Мы храним **срок окончания** Pro, чтобы применять лимиты и оказывать поддержку. Статус Pro может **кэшироваться локально** на устройстве (например, чтобы показывать дату окончания без сети). **Каждый лицензионный ключ можно активировать только на одном устройстве**; повторная активация того же ключа на другом устройстве не поддерживается.

**Подписки (App Store / Google Play):** если мы предложим в-app подписки, **приём платежей** и базовое управление подпиской выполняются **Apple** и/или **Google** на их условиях. Мы можем использовать **сторонний сервис статуса подписки** (например, для проверки покупок, продлений и отмен), чтобы включать или продлевать функции Pro на вашем устройстве. Такой сервис обрабатывает данные по своей политике конфиденциальности; мы используем его только для предоставления купленных вами прав. Мы обновим эту Политику при запуске подписок.
